...
export default function Weather(props) {
    ...
    const calcTemp = (index) => {
        const temp = props.weather.daily[index].temp;
        return Math.ceil(((temp.eve + temp.max) / 2) - 272.15);
    };

    // const getImgByTemp = (index) => {
    //     const weather = props.weather.daily[index];
    //     let image = "rainbow";
    ...
    const getImgByTemp = (index) => {
        if (!props.weather || !props.weather.daily || !props.weather.daily[index]) {
            console.error(`Invalid weather data or index out of bounds: index=${index}`, props.weather);
    ...
    const getColorFeelsLike = (index) => {
        let count = 0;
        if (props.weather) {
            const tempFeels = props.weather.daily[index].feels_like;
            const temp = props.weather.daily[index].temp;
    ...
    return (
        props.weather.daily && <div className={styles.center}>
